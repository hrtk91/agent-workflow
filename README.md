...
runの実行履歴と実行中の状況を端末上で確認するには`aw ui`を使います。
...
トップ画面はrunのトリアージに専念します。状態、開始時刻、経過時間、run識別子、
現在stepまたは失敗stepを一覧で確認でき、`f`で`すべて`・`実行中`・`失敗・要確認`・`成功`を切り替えられます。
矢印キーまたは`j`／`k`でrunを選び、`Enter`または`l`で詳細画面を開きます。
`:`を押すとコマンドを入力できます。

詳細画面は左にstep一覧、右に選択stepのログを表示する2カラム構成です。初期フォーカスは
左のstep一覧にあり、`j`／`k`でstepを選びます。`l`で右のログへ移り、ログ側では`j`／`k`で
スクロールします。`h`でstep一覧へ戻り、左のstep一覧で`h`を押すとトップ画面へ戻ります。
stdout・stderrは`Tab`／`o`／`e`で切り替えます。色は補助として使い、記号と日本語ラベルでも状態を判別できます。
実行中のログは自動的に末尾へ追従します。`k`または`g`で過去ログを確認する位置に固定し、`G`で末尾追従へ戻せます。
表示は既定で1秒ごとに更新され、`--refresh-seconds`で間隔を変更できます。
...
メニューとコマンドパレットでは、filter変更・更新・詳細表示・ヘルプ・終了を実行できます。
`resume`、`retry`、`cleanup`など状態を変更する操作は、誤操作を避けるためCLIから実行します。
