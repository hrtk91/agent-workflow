...
画面では、矢印キーまたは`j`／`k`でrunまたはqueue jobを選び、Enterで本格的なワークスペースを開きます。
トップ画面で`l`を押すと一覧を残したまま右ペインがログドリルダウンに切り替わり、
選択中のrunのstepログ末尾を確認できます。`l`または`Esc`でパイプライン表示に戻ります。
`:`を押すとコマンドを入力できます。

stepは状態ごとに色分けし、実行中のstepを反転と太字で強調します。処理順は矢印でつなぎ、
色に対応していない端末でも記号と日本語ラベルで状態を判別できます。詳細画面では
`✅`、`🚀`、`❌`などの絵文字も状態表示に併用します。
...
メニューとコマンドパレットは同じ操作を実行します。トップ画面のログドリルダウンでは、
`[`／`]`でstepを切り替え、`Tab`／`o`／`e`でstdout・stderrを切り替えます。`Enter`で
本格的なrunワークスペースへ移動できます。runワークスペースでは、stepを選び、
